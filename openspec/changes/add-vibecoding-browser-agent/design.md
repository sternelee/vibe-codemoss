...
- Browser Dock child WebView 必须与主窗口 Composer drag-drop contract 解耦：Tauri/Wry 会按 WebView label 分发 OS drag/drop，因此 child WebView 截获外部文件/文件夹 drop 时，后端必须把 `DragDropEvent` 转发给 main window 的 drag-drop service。
...
[Risk] Browser Dock child WebView 截获 OS 文件/文件夹拖拽，导致 Composer 无法收到外部路径。  
Mitigation：全局监听 Tauri `WebviewEvent::DragDrop`，将 child WebView 的 drop payload 转发到 main window；frontend drag-drop service 同时订阅 main native event 与 forwarded event，并继续复用 Composer file-reference pipeline。
