...
结论：当前方向没有跑偏，但实现状态已经分成两个清晰层次：`API view / UI substrate` 已进入可读可交互阶段，`API discovery / parser adapter substrate` 仍是下一批 P0 深水区。

### 当前完成度 / Current progress

- OpenSpec task progress：`13 / 48`。
- 已完成主链路：
...
|---|---|---|
| 独立 API contract scan branch | 已开始 | 对齐。API branch 与 file relationship branch 保持隔离，但 branch status、scope、merge 仍需补强。 |
| 通用 API contract graph | 已开始 | 对齐。UI 已消费统一 graph model；data model / ownership / stale / merge 仍需闭环。 |
| 多语言 adapter 架构 | 未完成 | 方向对齐但尚未完成。下一阶段必须先补 adapter skeleton 与 explicit unsupported / no-candidate reason。 |
| 强契约源优先 | 未完成 | 仍是正确方向。OpenAPI / protobuf / GraphQL adapter 还未闭环。 |
| group-first API view | 已实现 Alpha | 对齐。默认不平铺大规模 endpoint。 |
| API Inspector | 已实现 Alpha | 对齐。method chain inspector 与 filter/search hierarchy reveal 仍未完成。 |
| evidence/confidence/redaction | 已开始 | 对齐。redaction utility 已完成，弱推断的 parser/evidence provider 仍需补全。 |
...
- 需要保留为风险：Java / Kotlin、Python、Go、C、C++、TS/JS、C#、Rust adapter skeleton 尚未形成完整 matrix，不能宣称多语言扫描已完成。
...
`API View Alpha: contract graph presentation + group-first drill-down + endpoint inspector`

它已经满足“用户能看到 API tab、理解 group-first 接口图、从分组进入 endpoint、查看接口详情和 redacted evidence”的阶段目标；
但还不能定义为完整 API discovery 交付，因为 parser adapter、endpoint merge、method chain、scan scope 与 ownership/stale gate 仍未闭环。

### 下一阶段建议 / Next calibrated batch

1. 优先完成 `1.1 / 1.3 / 2.2 / 2.3 / 2.4 / 4.0`，把 data model、ownership、scope、identity merge、adapter skeleton 先闭环。
2. 再推进 OpenAPI / protobuf / GraphQL 强契约 adapter，避免先陷入弱源码推断。
3. 完成 method chain inspector 与 filter/search hierarchy reveal，补齐 API view 的可解释链路与定位能力。
