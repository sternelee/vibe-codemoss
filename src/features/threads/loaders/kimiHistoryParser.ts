...
function extractKimiAssistantFinalFlag(message: Record<string, unknown>): boolean | undefined {
  const metadata =
...
function markKimiAssistantFinalMessages(items: ConversationItem[]) {
...
function hydrateKimiAssistantFinalTiming(
...
function normalizeKimiToolSnapshotType(rawToolType: string): string {
...
function normalizeKimiToolStartItem(
  message: Record<string, unknown>,
  itemId: string,
  rawToolType: string,
  text: string,
): Extract<ConversationItem, { kind: "tool" }> | null {
  const normalizedToolType = normalizeKimiToolSnapshotType(rawToolType);
...
export function parseKimiHistoryMessages(messagesData: unknown): ConversationItem[] {
  ...
  for (const entry of messagesData) {
    ...
    const itemId =
      asString(message.id ?? "").trim() || `kimi-history-item-${items.length + 1}`;
    ...
    if (kind === "message") {
      ...
      const assistantFinalFlag =
        role === "assistant"
          ? extractKimiAssistantFinalFlag(message)
          : undefined;
    ...
    const normalizedStartItem = normalizeKimiToolStartItem(
      message,
      itemId,
      rawToolType,
      text,
  ...
  markKimiAssistantFinalMessages(items);
  hydrateKimiAssistantFinalTiming(items, messageTimestampById);
