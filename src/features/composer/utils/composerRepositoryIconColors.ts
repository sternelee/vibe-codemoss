...
export const COMPOSER_REPOSITORY_ICON_COLOR_CLASSES = [
...
export function buildComposerRepositoryIconColorSlots(
  ...
  sortedRepositoryRoots.forEach((repositoryRoot) => {
    let colorSlot = getRepositoryColorHash(repositoryRoot)
      % COMPOSER_REPOSITORY_ICON_COLOR_CLASSES.length;
    while (
      usedColorSlots.size < COMPOSER_REPOSITORY_ICON_COLOR_CLASSES.length
      && usedColorSlots.has(colorSlot)
    ) {
      colorSlot = (colorSlot + 1) % COMPOSER_REPOSITORY_ICON_COLOR_CLASSES.length;
    }
